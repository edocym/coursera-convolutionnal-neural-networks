...
def compute_content_cost(content_output, generated_output):
    ...
    # If you are transposing, Transpose both tensor againnst the same dimensions.
    a_C_unrolled = tf.transpose(tf.reshape(tensor = a_C, shape = [m, n_H * n_W, n_C]))      # same as tf.reshape(a_C, shape=[m, -1, n_C])
    a_G_unrolled = tf.transpose(tf.reshape(tensor = a_G, shape = [m, -1, n_C]))
    # a_C_unrolled = tf.transpose(tf.reshape(tensor = a_C, shape = [m, n_H * n_W, n_C]), perm = [0, 1, 2])      # same as tf.reshape(a_C, shape=[m, -1, n_C])
    # a_G_unrolled = tf.transpose(tf.reshape(tensor = a_G, shape = [m, -1, n_C]), perm = [0, 1, 2])
...
def gram_matrix(A):
    ...
    # GA = tf.matmul(A, A, transpose_a = False, transpose_b = True)     # same result as above
...
def compute_layer_style_cost(a_S, a_G):
    ...
    a_S = tf.transpose(tf.reshape(tensor = a_S, shape = [n_H * n_W, n_C]))
    a_G = tf.transpose(tf.reshape(tensor = a_G, shape = [n_H * n_W, n_C]))
...
optimizer = tf.keras.optimizers.Adam(learning_rate=0.001)    # was learning_rate = 0.03
