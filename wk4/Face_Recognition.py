...
def triplet_loss(y_true, y_pred, alpha = 0.2):
    ...
    pos_dist = tf.reduce_sum(input_tensor = tf.square(tf.subtract(anchor, positive)), axis = -1, keepdims = False)
    # Step 2: Compute the (encoding) distance between the anchor and the negative
    neg_dist = tf.reduce_sum(input_tensor = tf.square(tf.subtract(anchor, negative)), axis = -1, keepdims = False)
    # Step 3: subtract the two previous distances and add alpha.
    basic_loss = tf.add(tf.subtract(pos_dist, neg_dist), alpha)
    # Step 4: Take the maximum of basic_loss and 0.0. Sum over the training examples.
    loss = tf.reduce_sum(input_tensor = tf.maximum(basic_loss, 0), axis = None, keepdims = False)
    ### END CODE HERE
