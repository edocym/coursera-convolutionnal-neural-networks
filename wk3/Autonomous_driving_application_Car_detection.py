...
def yolo_filter_boxes(boxes, box_confidence, box_class_probs, threshold = .6):
    ...
    ##(≈ 2 lines)
    # box_classes = tf.math.argmax(input = box_scores, axis = -1)    #also would work axis = -1
    # box_class_scores = tf.math.reduce_max(input_tensor = box_scores, axis = -1, keepdims = False)
    box_classes = tf.math.argmax(box_scores, axis = -1)    #also would work axis = -1
    ...
    ## (≈ 3 lines)
    scores = tf.boolean_mask(tensor = box_class_scores, mask = filtering_mask, axis = None)
    boxes = tf.boolean_mask(tensor = boxes, mask = filtering_mask, axis = None)
    classes = tf.boolean_mask(tensor = box_classes, mask = filtering_mask, axis = None)
    ### END CODE HERE
